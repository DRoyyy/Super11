...
function App() {
  ...
  const handleDeletePlayer = (id, price) => {
    const newSelectedPlayersList = selectedPlayersList.filter((p) => p.id !== id);
    setSelectedPlayersList(newSelectedPlayersList);
  ...
  return (
    <>
      <Header addFreeCredit={addFreeCredit} credit={credit} creditClaimed={creditClaimed}></Header>
      <div className='flex items-center justify-center max-w-screen-2xl mx-auto'>
          <button onClick={() => handleShowAvailable(true)} className={`font-semibold px-5 py-3 border rounded-l-lg ${showAvailable ? "bg-yellow-400"  : "bg-white"}`}>Available</button>
          <button onClick={() => handleShowAvailable(false)} className={`font-semibold px-5 py-3 border rounded-r-lg ${!showAvailable ? "bg-yellow-400" : "bg-white"}`}>Selected</button>
      </div>
      <div className='relative'>
        {
          showAvailable ? <Available handleSelectedPlayer={handleSelectedPlayer}></Available> 
          : <SelectedPlayersList selectedPlayersList={selectedPlayersList} handleDeletePlayer={handleDeletePlayer} handleShowAvailable={handleShowAvailable}></SelectedPlayersList>
        }
